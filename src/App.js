...
const App = () => {
  ...
  let userInputHander = (userInput) => {
    if (userInput === '') {
      ...
      localStorage.setItem(
        'todolist',
        JSON.stringify([
          { text: userInput, doneStatus: false, taskId: taskId },
          ...arrayContainer,
        ])
      );

      //to combine the data from the local storage and new data
      setArrayContainer((ps) => [
        { text: userInput, doneStatus: false, taskId: taskId },
        ...ps,
      ]);
  ...
  let checkBoxHandler = (toCheckElement, condition) => {
    if (condition === true) {
      let updatedArray = arrayContainer.map((element) => {
        if (element.taskId === toCheckElement.taskId)
          //if exist then make it true
  ...
  let forEmptyEditedElement = (id) => {
    setEditElement(
      (
        ps //to keep the form display until the any input
      ) => {
        return {
          ...ps,
          text: [],
  ...
  const editInputSubmitter = (id) => {
    if (elementToEdit.text.length === 0) {
      ...
      let updatedTodo = arrayContainer.map((element) => {
        if (element.taskId === elementToEdit.taskId)
          //updated the target task
  ...
  return (
    <div className="todo_part ">
      <label className="heading  text-center  w-100 text-light mb-5 ">
        {' '}
        📝Task Tracker✍
      </label>
      <div className="container w-100 text-bg-light pt-4">
        <div
          className={`fw-bold text-center alert lead ${color} lead ${display} `}
          role="alert"
        >
          {msg}
        </div>
        ...
        <FullTodoBox
          ...
          userInputHander={userInputHander}
          checkBoxHandler={checkBoxHandler}
          deleteTodo={deleteTodo}
          clearAll={clearAll}
          firsrEditTodoReceiver={firsrEditTodoReceiver}
          elementToEdit={elementToEdit}
          changeEditInputHandler={changeEditInputHandler}
          editInputSubmitter={editInputSubmitter}
          forEmptyEditedElement={forEmptyEditedElement}
        />
      </div>
    </div>
  );
